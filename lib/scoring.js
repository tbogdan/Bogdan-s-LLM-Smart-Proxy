...
function detectIncompatibility(providerName, statusCode, errorBody) {
  ...
  // Gemini thought_signature missing — retry with dummy signature injected
  if (body.includes("thought_signature") && statusCode === 400) {
    state.log(`COMPAT: ${providerName} requires thought_signature — will inject dummy on retry`);
  }
