...
function detectIncompatibility(providerName, statusCode, errorBody) {
  ...
  // Gemini thought_signature missing — Gemini requires thought_signature in tool calls, we can't provide it
  if (body.includes("thought_signature") && statusCode === 400) {
    setCooldown(providerName, 300_000); // 5 min cooldown — let other providers handle tools
    state.log(`COMPAT: ${providerName} requires thought_signature in tool calls — cooldown 5min`);
  }
...
function recordStalling(name) {
  ...
  if (state.stallingTracker[name].length >= 2) {
    if (!isOnCooldown(name)) {
      setCooldown(name, 3600_000);
      state.log(`STALL-COOLDOWN: ${name} stalled 2+ in 5min — cooldown 1h`);
    }
