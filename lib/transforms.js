...
function transformRequest(provider, reqBody) {
  ...
  if (body.messages && body.messages.length > 0) {
    ...
    let PROXY_SYSTEM = [
      ...
      "SAVE: Save progress to MemPalace with file paths and what's done. Mark subtask complete in todo.",
      "NEXT: Immediately start next subtask. NEVER stop between subtasks. On 'continue' = read todo + mempalace, resume next incomplete subtask.",
      "",
      ...
      "- After each subtask: save to MemPalace what changed. Mark complete in todo.",
      ...
      "- Use MemPalace as shared ledger: save subagent task + result to mempalace. Next subagent reads previous results before starting.",
  ...
  // Detect tool call loops — same tool+args repeated 3+ times → inject warning
  if (body.messages) {
    const toolCallCounts = {};
    for (const m of body.messages) {
      if (m.role === "assistant" && m.tool_calls) {
        for (const tc of m.tool_calls) {
          const key = (tc.function?.name || "") + ":" + (tc.function?.arguments || "").substring(0, 100);
          toolCallCounts[key] = (toolCallCounts[key] || 0) + 1;
        }
      }
    }
    const loops = Object.entries(toolCallCounts).filter(([, c]) => c >= 3);
    if (loops.length > 0) {
      const loopNames = loops.map(([k, c]) => `${k.split(":")[0]}(${c}x)`).join(", ");
      state.log(`TOOL-LOOP: detected repeated tool calls: ${loopNames}`);
      // Inject warning as last system-like message before the last user message
      const lastUserIdx = body.messages.findLastIndex(m => m.role === "user");
      if (lastUserIdx > 0) {
        body.messages.splice(lastUserIdx, 0, {
          role: "assistant",
          content: `WARNING: You are in a tool call loop (${loopNames}). You already read this file/data multiple times. STOP re-reading. Use the information you already have. If the file content was already provided in conversation, do NOT read it again. Proceed to the NEXT step: edit, write, or act on what you know.`,
        });
      }
      // Also deduplicate: keep only last occurrence of each repeated tool call + response pair
      if (loops.some(([, c]) => c >= 5)) {
        const seen = {};
        const filtered = [];
        for (let i = body.messages.length - 1; i >= 0; i--) {
          ...
          if (m.role === "assistant" && m.tool_calls?.length === 1) {
            ...
            const key = (tc.function?.name || "") + ":" + (tc.function?.arguments || "").substring(0, 100);
            if (toolCallCounts[key] >= 5) {
              if (seen[key]) continue; // skip older duplicate
              seen[key] = true;
            }
          }
          // Also skip duplicate tool responses
          if (m.role === "tool" && m.tool_call_id) {
            const prevAssist = body.messages.slice(0, i).reverse().find(x => x.role === "assistant" && x.tool_calls?.some(tc => tc.id === m.tool_call_id));
            if (!prevAssist) { filtered.unshift(m); continue; } // orphan, keep
            const tc = prevAssist.tool_calls.find(tc => tc.id === m.tool_call_id);
            const key = (tc?.function?.name || "") + ":" + (tc?.function?.arguments || "").substring(0, 100);
            if (toolCallCounts[key] >= 5 && !seen[key + ":resp"]) {
              seen[key + ":resp"] = true;
              filtered.unshift(m);
              continue;
            } else if (toolCallCounts[key] >= 5) continue; // skip older duplicate response
          }
          filtered.unshift(m);
        }
        const loopRemoved = body.messages.length - filtered.length;
        if (loopRemoved > 0) {
          body.messages = filtered;
          state.log(`TOOL-LOOP: deduplicated ${loopRemoved} repeated tool call/response pairs`);
        }
      }
    }
  }
...
function detectUseCase(messages, reqBody) {
  ...
  return { caps, isAgent, needsUpgrade };
}
