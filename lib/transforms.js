...
function transformRequest(provider, reqBody) {
  ...
  // Sanitize tool definition names — Gemini requires [a-zA-Z_][a-zA-Z0-9_.:-]* max 128 chars
  if (body.tools) {
    body.tools = body.tools.filter(t => t.function?.name).map(t => {
      const name = t.function.name.replace(/[^a-zA-Z0-9_.\-:]/g, "_").substring(0, 128);
      if (name !== t.function.name) return { ...t, function: { ...t.function, name } };
      return t;
    });
  }
