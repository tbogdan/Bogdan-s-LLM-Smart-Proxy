...
//
// NOTE on union completeness: seed-providers.json covers the 4 supported
// provider families (windsurf, claude, codex, copilot). Legacy families
// (OpenAI, Groq, Ollama, OpenRouter, Cloudflare, etc.) are env-driven and
// loaded from /data/providers.json written by discovery — NOT seeded here.
// This is intentional: legacy family keys/URLs vary per deployment.
// ---------------------------------------------------------------------------
function hydrateSeedProviders(config) {
  ...
  for (const p of (config.providers || [])) {
    // Deduplication guard: skip providers with duplicate names (first occurrence
    // is preferred as canonical; subsequent entries with same name are dropped).
    if (p.name && seenNames.has(p.name)) {
      state.log(`WARN: Skipping seed provider with duplicate name "${p.name}" (model: ${p.model})`);
      continue;
    }
