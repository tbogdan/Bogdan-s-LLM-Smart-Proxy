...
async function processRequest(request, emitEvent, emitError, emitDone, conversationHistory) {
  ...
  if (conversationHistory && conversationHistory.length > 0) {
    ...
    // Cap history to prevent unbounded growth (keep system + last 400 messages)
    while (conversationHistory.length > 400 && conversationHistory[0]?.role !== "system") {
  ...
  const mockRes = new MockResponse(
    ...
    () => {
      ...
      if (conversationHistory && mockRes._textContent && Object.keys(mockRes._toolCalls).length === 0) {
        conversationHistory.push({ role: "assistant", content: mockRes._textContent });
      } else if (conversationHistory && Object.keys(mockRes._toolCalls).length > 0) {
...
function handleResponsesWS(ws, req) {
  state.log(`RESPONSES-WS: connection from ${req.socket.remoteAddress}`);
  const conversationHistory = []; // accumulates chat completion messages across turns
  const pendingItems = []; // items added via conversation.item.create before response.create
  ...
  ws.on("message", async (data) => {
    ...
    try { msg = JSON.parse(raw); } catch {
      state.log(`RESPONSES-WS: JSON parse error`);
      ws.send(JSON.stringify({ type: "error", error: { message: "Invalid JSON" } }));
      return;
    }

    const type = msg.type;
    state.log(`RESPONSES-WS: ${type} ${raw.length}chars`);

    // session.update — save config for future turns
    if (type === "session.update") {
      if (msg.session?.model) sessionModel = msg.session.model;
      if (msg.session?.instructions) sessionInstructions = msg.session.instructions;
      if (msg.session?.tools) sessionTools = msg.session.tools;
      // Acknowledge
      if (ws.readyState === 1) ws.send(JSON.stringify({ type: "session.updated", session: msg.session || {} }));
      return;
    }

    // conversation.item.create — accumulate items for next response.create
    if (type === "conversation.item.create") {
      const item = msg.item;
      if (item) {
        pendingItems.push(item);
        state.log(`RESPONSES-WS: queued item type=${item.type} role=${item.role || ""}`);
        // Acknowledge
        if (ws.readyState === 1) ws.send(JSON.stringify({ type: "conversation.item.created", item }));
      }
      return;
    }

    // response.create — trigger inference
    if (type === "response.create") {
      // Merge: response wrapper OR flat format
      let request = msg.response ? { ...msg.response, model: msg.response.model || msg.model } : { ...msg };
      delete request.type;

      // Merge pending items into input
      const existingInput = request.input || [];
      request.input = [...pendingItems, ...existingInput];
      pendingItems.length = 0; // clear pending

      // Apply session defaults
      if (!request.model && sessionModel) request.model = sessionModel;
      if (!request.instructions && sessionInstructions) request.instructions = sessionInstructions;
      if (!request.tools && sessionTools) request.tools = sessionTools;

      state.log(`RESPONSES-WS: response.create model=${request.model} input=${request.input.length} tools=${request.tools?.length || 0}`);
    ...
    state.log(`RESPONSES-WS: unknown event type=${type}`);
  });
