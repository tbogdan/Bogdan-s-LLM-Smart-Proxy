...
function translateInput(request) {
  ...
  if (tools?.length > 0 && tools[0].name && !tools[0].function) {
    tools = tools.map(t => ({
      type: "function",
      function: {
        name: t.name,
...
// ---------------------------------------------------------------------------
async function processRequest(request, emitEvent, emitError, emitDone) {
  const chatReq = translateInput(request);
  state.log(`RESPONSES: model=${request.model}→${chatReq.model} input=${request.input?.length || 0} items → ${chatReq.messages.length} msgs tools=${chatReq.tools?.length || 0}`);

  const mockRes = new MockResponse(emitEvent, emitError, emitDone);

  try {
    await _handleChatCompletion(chatReq, mockRes);
  } catch (err) {
...
function handleResponsesWS(ws, req) {
  ...
  ws.on("message", async (data) => {
    ...
    // Handle response.create wrapper (Realtime API format)
    if (request.type === "response.create" && request.response) {
