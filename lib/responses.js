...
function handleResponsesWS(ws, req) {
  ...
  ws.on("message", async (data) => {
    ...
    state.log(`RESPONSES-WS: type=${request.type} model=${request.model || request.response?.model} input=${request.input?.length || request.response?.input?.length || 0} tools=${request.tools?.length || request.response?.tools?.length || 0}`);
