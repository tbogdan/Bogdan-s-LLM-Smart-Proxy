...
async function handleChatCompletion(reqBody, clientRes) {
  ...
  // Early tool loop dedup on reqBody — cleans before transform + caching
  // Keeps only last occurrence of tool calls repeated 5+ times
  if (reqBody.messages && reqBody.messages.length > 20) {
    const tcCounts = {};
    for (const m of reqBody.messages) {
      if (m.role === "assistant" && m.tool_calls) {
        for (const tc of m.tool_calls) {
          const k = (tc.function?.name || "") + ":" + (tc.function?.arguments || "").substring(0, 100);
          tcCounts[k] = (tcCounts[k] || 0) + 1;
        }
      }
    }
    const loopKeys = new Set(Object.entries(tcCounts).filter(([, c]) => c >= 5).map(([k]) => k));
    if (loopKeys.size > 0) {
      // Walk backwards — keep last occurrence of each looped call+response, drop earlier ones
      const seen = {};
      const cleaned = [];
      for (let i = reqBody.messages.length - 1; i >= 0; i--) {
        ...
        if (m.role === "assistant" && m.tool_calls?.length === 1) {
          ...
          const k = (tc.function?.name || "") + ":" + (tc.function?.arguments || "").substring(0, 100);
          if (loopKeys.has(k)) {
            if (seen[k]) continue; // skip older dup
            seen[k] = true;
          }
        }
        if (m.role === "tool" && m.tool_call_id) {
          // Find matching assistant call to check if it's a loop
          const matchAssist = reqBody.messages.slice(0, i).reverse().find(x =>
            x.role === "assistant" && x.tool_calls?.some(tc => tc.id === m.tool_call_id));
          if (matchAssist) {
            const tc = matchAssist.tool_calls.find(tc => tc.id === m.tool_call_id);
            const k = (tc?.function?.name || "") + ":" + (tc?.function?.arguments || "").substring(0, 100);
            if (loopKeys.has(k) && seen[k + ":r"]) continue; // skip older dup response
            if (loopKeys.has(k)) seen[k + ":r"] = true;
          }
        }
        cleaned.unshift(m);
      }
      const removed = reqBody.messages.length - cleaned.length;
      if (removed > 0) {
        reqBody.messages = cleaned;
        state.log(`LOOP-DEDUP-EARLY: removed ${removed} repeated tool call pairs from reqBody (${[...loopKeys].map(k => k.split(":")[0]).join(",")})`);
      }
    }
  }
    ...
    if (providers.length <= 3 && reqBody.messages?.length > 10) {
      ...
      if (allCount > providers.length * 2) {
        ...
        for (const pct of layers) {
          ...
          const compacted = compaction.compactMessages(originalMessages, target, requestedMaxTokens, []);
