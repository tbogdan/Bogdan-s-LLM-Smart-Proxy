...
async function handleChatCompletion(reqBody, clientRes) {
  ...
  const lastUserMsg = reqBody.messages?.[reqBody.messages.length - 1];
  const lastUserContent = (lastUserMsg?.role === "user" && typeof lastUserMsg.content === "string") ? lastUserMsg.content.trim() : "";
  const banMatch = lastUserContent.match(/^ban\s+ai\s+(\d+)\s*$/i);
  if (banMatch) {
