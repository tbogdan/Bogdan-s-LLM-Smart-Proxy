...
function buildUsageHeaders(provider, reqBody, usage) {
  ...
  const sid = (reqBody._session || memory.getSession(reqBody.messages))?.id;
  const ss = sid ? state.sessionStats[sid] : null;

  // Routing-policy classification headers
  let policyProfile = null;
  try { policyProfile = routingPolicy.classifyProvider(provider); } catch {}
  ...
  const headers = {
    ...
    ...(policyProfile ? {
      "X-LLM-Cost-Group": String(policyProfile.cost_group),
      "X-LLM-Speed-Group": String(policyProfile.speed_group),
      "X-LLM-Thinking-Effort": String(policyProfile.thinking_effort || ""),
    } : {}),
    ...(ss ? {
...
function getProvidersForGroup(groupName, estimatedTokens, reqBody) {
  ...
  const scored = withContext.map((p) => {
    ...
    // Routing-policy bonus — uses policy targets (cost, quality, speed) for fine-grained routing
    try {
      bonus += routingPolicy.providerPolicyBonus(p, reqBody, { getEffectiveContext: scoring.getEffectiveContext });
    } catch {}
...
function getProvidersForAuto(messages, estimatedTokens, reqBody) {
  ...
  const scored = withContext.map((p) => {
    ...
    // Routing-policy bonus — uses policy targets for fine-grained routing
    try {
      bonus += routingPolicy.providerPolicyBonus(p, reqBody, { getEffectiveContext: scoring.getEffectiveContext });
    } catch {}
...
async function routeToProvider(provider, reqBody) {
  ...
  {
    const sess = reqBody._session || memory.getSession(reqBody.messages);
    const sid = sess.id;
    ...
    try { memory.setLastProvider(sess, provider.name); } catch {}
    // Track in sessionStats for reliable cross-request lookups
    try { if (state.sessionStats[sid]) state.sessionStats[sid].lastProvider = provider.name; } catch {}
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const streamReq = streamRequest(
      ...
      (chunk) => {
        ...
        // Skip all writes after loop abort
        if (streamLoopAborted) return;
      ...
      () => {
        ...
        try {
          ...
          _trackSessionTokens(sid, inTok, outTok, provider.name);
          memory.setLastProvider(sess, provider.name);
          if (state.sessionStats[sid]) state.sessionStats[sid].lastProvider = provider.name;
        } catch {}
...
async function handleChatCompletion(reqBody, clientRes) {
  ...
  if (banMatch) {
    ...
    const banSid = memory.getSession(reqBody.messages)?.id;
  ...
  const sid = memory.getSession(reqBody.messages)?.id;
  ...
  // Memory: recall memories and inject into context
  const mpSession = memory.getSession(reqBody.messages);
  reqBody._session = mpSession; // cache session on reqBody so routeToProvider/routeStreamToProvider avoid repeated getSession() calls

  // Track session tokens
  _trackSessionTokens(mpSession.id, currentTokens, 0);
  // Get last provider from session stats
  const ss = state.sessionStats[mpSession.id];
  reqBody._sessionLastProvider = ss?.lastProvider || memory.getLastProvider(mpSession);
  let memoryInjection = "";
  try {
    memoryInjection = await memory.recallMemories(mpSession);
  ...
  if (mpSession.requestCount === 1) {
    ...
    if (sysMsg) {
      ...
      memory.saveProjectContext(mpSession, sysContent);
    }
  }
  ...
  if (isGroup) {
    ...
    for (const provider of providers) {
      ...
      try {
        ...
        memory.saveError(mpSession, `${provider.name} HTTP ${status}: ${errFull}`);
