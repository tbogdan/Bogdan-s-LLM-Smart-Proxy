...
function buildUsageHeaders(provider, reqBody, usage) {
  ...
  const sid = session.getSession(reqBody.messages)?.id;
...
async function routeToProvider(provider, reqBody) {
  ...
  try { session.setLastProvider(session.getSession(reqBody.messages), provider.name); } catch {}
  // Track in sessionStats for reliable cross-request lookups
  try { const sid = session.getSession(reqBody.messages).id; if (state.sessionStats[sid]) state.sessionStats[sid].lastProvider = provider.name; } catch {}
  ...
  return new Promise((resolve, reject) => {
    ...
    const streamReq = streamRequest(
      ...
      () => {
        ...
        try { session.setLastProvider(session.getSession(reqBody.messages), provider.name); } catch {}
        try { const sid2 = session.getSession(reqBody.messages).id; if (state.sessionStats[sid2]) state.sessionStats[sid2].lastProvider = provider.name; } catch {}
...
async function handleChatCompletion(reqBody, clientRes) {
  ...
  if (banMatch) {
    ...
    const banSid = session.getSession(reqBody.messages)?.id;
  ...
  const mpSession = session.getSession(reqBody.messages);
  ...
  reqBody._sessionLastProvider = ss?.lastProvider || session.getLastProvider(mpSession);
  let memoryInjection = "";
  try {
    memoryInjection = "";
