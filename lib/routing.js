...
async function handleChatCompletion(reqBody, clientRes) {
  ...
  // These need to go through even when context is huge (that's the whole point)
  ...
  // BUT: if it's a summarization request, compact aggressively and let it through.
  if (isGroup && reqBody.messages?.length > 20) {
    ...
    if (bestCtx > 0 && currentTokens > bestCtx * 0.8) {
      if (isSummarizationRequest) {
        // IDE is asking for summarization — compact aggressively and route normally
        const target = Math.floor(bestCtx * 0.5); // 50% of best context — leave room for response
        state.log(`SUMMARIZE-COMPACT: input ${currentTokens}tok > 80% of best ctx ${bestCtx}, but summarization request detected — compacting to ${target}tok`);
        const compacted = compaction.compactMessages(reqBody.messages, target, requestedMaxTokens, []);
        if (compacted) {
          reqBody.messages = compacted.messages;
          reqBody._estimatedTokens = compaction.estimateTokens(reqBody.messages) + compaction.estimateToolsTokens(reqBody.tools) + requestedMaxTokens;
          state.log(`SUMMARIZE-COMPACT: ${currentTokens}tok → ${compaction.estimateTokens(reqBody.messages)}tok (${compacted.removed} msgs removed)`);
        }
        // Fall through to normal routing
      } else {
        state.log(`IDE-COMPACT: input ${currentTokens}tok > 80% of best ctx ${bestCtx} — returning context_length_exceeded`);
    ...
    if ((continuaCount >= 2 || shortAssistantCount >= 3) && continuaCount + shortAssistantCount >= 3) {
      ...
      for (let i = msgs.length - 2; i >= Math.max(0, msgs.length - 8); i--) {
        ...
        if (m.role === "assistant" && !m.tool_calls?.length) {
          ...
          let fakeExec = 0;
          for (const pat of transforms.FAKE_EXEC_PATTERNS) { if (pat.test(aContent)) fakeExec++; }
          if (fakeExec >= 1 || aContent.length < 500) {
            ...
            if (continuaCount >= 3) {
              ...
              for (const tp of topProviders) {
                scoring.recordStalling(tp.name);
                scoring.banFromGroup(tp.name, banGroup);
                mempalace.saveStalling(tp.name, banGroup, "repeated stalling");
              }
              state.log(`STALL-BAN: ${continuaCount}x continua, banned ${topProviders.map((p) => p.name).join(", ")} from ${banGroup} (${state.groupBans[banGroup]?.size || 0} total banned)`);
            } else {
              // 2x continua = 5min cooldown only
              for (const tp of topProviders) {
                scoring.recordStalling(tp.name);
                scoring.setCooldown(tp.name, 5 * 60_000);
              }
              state.log(`STALL-COOL: ${continuaCount}x continua, ${topProviders.map((p) => p.name).join(", ")} cooled 5min`);
            }
            break;
          }
        }
