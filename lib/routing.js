...
async function handleChatCompletion(reqBody, clientRes) {
  ...
  // Check last 3 messages for a user message with ban command (IDE may append system msgs after)
  let banMatch = null;
  for (let bi = (reqBody.messages?.length || 0) - 1; bi >= Math.max(0, (reqBody.messages?.length || 0) - 3); bi--) {
    const m = reqBody.messages[bi];
    if (m?.role !== "user") continue;
    // Handle both string content and content array [{type:"text", text:"..."}]
    let text = "";
    if (typeof m.content === "string") text = m.content.trim();
    else if (Array.isArray(m.content)) text = m.content.filter(p => p.type === "text").map(p => p.text).join(" ").trim();
    banMatch = text.match(/^ban\s+ai\s+(\d+)\s*$/i);
    if (banMatch) break;
  }
  if (banMatch) {
