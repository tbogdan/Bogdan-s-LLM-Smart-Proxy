...
async function handleChatCompletion(reqBody, clientRes) {
  ...
  // Intercept "ban ai XX" — regex scan ALL messages for the command
  ...
  if (banMatch) {
    ...
    if (banProvider && banHours > 0) {
      ...
      state.log(`USER-BAN: no provider to ban (session: ${banSid}, lastProvider: ${banProvider})`);
    }

    // Strip ALL "ban ai XX" from messages so it never reaches provider (even from IDE cache)
    ...
    // Return confirmation
    const msg = banProvider && banHours > 0
      ? `Banned **${banProvider}** for ${banHours} hour${banHours > 1 ? "s" : ""}. Will use other providers.`
      : `No provider to ban — no previous request in this session.`;
    if (isStreaming) {
