...
async function handleChatCompletion(reqBody, clientRes) {
  ...
  // Intercept "ban ai XX" command — user wants to ban the last provider for XX hours
  // Scan last 5 messages for user message containing the command
  let banMatch = null;
  for (let bi = (reqBody.messages?.length || 0) - 1; bi >= Math.max(0, (reqBody.messages?.length || 0) - 5); bi--) {
    const m = reqBody.messages[bi];
    if (m?.role !== "user") continue;
    let text = "";
    if (typeof m.content === "string") text = m.content;
    else if (Array.isArray(m.content)) text = m.content.filter(p => p.type === "text").map(p => p.text).join(" ");
    // Extract just the user's actual text — strip IDE-injected tags
    text = text.replace(/<[^>]+>[^<]*<\/[^>]+>/g, "").replace(/<[^>]+\/>/g, "").trim();
    // Match "ban ai N" anywhere in the cleaned text (IDEs may add context around it)
    banMatch = text.match(/\bban\s+ai\s+(\d+)\b/i);
    if (banMatch && text.length < 500) break; // only if message is short (not embedded in long content)
    banMatch = null;
  }
  if (banMatch) {
    ...
    if (banProvider && banHours > 0) {
      ...
      sendBanResponse(`Banned **${banProvider}** for ${banHours} hour${banHours > 1 ? "s" : ""}. Will use other providers.`);
    } else {
      const errMsg = banProvider ? `Invalid ban duration.` : `No provider to ban — no previous request in this session.`;
      state.log(`USER-BAN: failed — ${errMsg}`);
      sendBanResponse(errMsg);
    }
    return;
  }
